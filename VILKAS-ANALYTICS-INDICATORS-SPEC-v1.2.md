...
**Billackering.eu** on auton hoitotuotteiden erikoisverkkokauppa, joka myy pääasiassa **paketteja** (tuotebundleja) yksittäisten tuotteiden sijaan.

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    BILLACKERING.EU LIIKETOIMINTAMALLI                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  TUOTESTRATEGIA:                                                        │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  📦 PAKETIT = Päätuotteet (~70% myynnistä)                      │   │
│  │     • Aloituspaketit (entry level)                              │   │
│  │     • Pro-paketit (harrastajat)                                 │   │
│  │     • Täydelliset paketit (kaikki kerralla)                     │   │
│  │     • Sesonkipaketit (talvi, kesä)                              │   │
│  │                                                                  │   │
│  │  🧴 YKSITTÄISET TUOTTEET (~30% myynnistä)                       │   │
│  │     • Täydennysostot                                            │   │
│  │     • Erikoistuotteet                                           │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  ASIAKASPOLKU:                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  1. Uusi asiakas → Aloituspaketti (matala kynnys)               │   │
│  │  2. Tyytyväinen → Pro-paketti tai täydennystuotteet             │   │
│  │  3. Harrastaja → Täydelliset paketit, erikoistuotteet           │   │
│  │  4. Palaava asiakas → Täydennysostot, uudet sesonkipaketit      │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  MARGINAALIRAKENNE:                                                     │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  • Paketit: Korkeampi kate (bundlaus)                           │   │
│  │  • Yksittäiset: Matalampi kate, mutta retention                 │   │
│  │  • Ostohinnat tiedossa → todellinen kannattavuus laskettavissa  │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
Tämä analytiikkajärjestelmä vastaa Billackering.eu:n näkökulmasta:

| Kysymys | Indikaattori | Datalähde |
|---------|--------------|-----------|
| Mitkä paketit myyvät parhaiten? | Package Performance | ePages |
| Mikä on pakettien todellinen kate? | **Profit Margin Indicator** | ePages (osto+myyntihinta) |
| Mistä hakusanoista paketit löytyvät? | Query Revenue Attribution | GSC + ePages |
| Onko pakettien komponentteja varastossa? | **Bundle Stock Risk** | ePages (saldot) |
| Mikä on asiakkaan elinkaari? | Customer Lifetime Value | ePages |
| Aloituspaketti → Pro-paketti konversio? | **Upsell Funnel** | ePages |
| Sesonkipaketit vs. ympärivuotiset? | Seasonal Performance | ePages |

### 0.3 Billackering.eu -spesifiset indikaattorit

Geneeristen indikaattorien lisäksi tarvitaan:

1. **Package Performance Indicator** - Pakettien myynti, kate, trendi
2. **Bundle Stock Risk** - Paketin komponenttien saatavuus
3. **Upsell Funnel** - Aloituspaketti → Pro → Täydellinen
4. **Profit Margin Indicator** - Todellinen kannattavuus (ostohinta vs. myyntihinta)
5. **Component Demand** - Mitkä yksittäiset tuotteet menevät paketeissa vs. erikseen
...
interface Product {
  ...
  // Hinnoittelu
  price_amount: number;           // Myyntihinta (€)
  cost_price: number;             // OSTOHINTA (€) - mahdollistaa katteen laskennan
  currency: string;
  
  // Varastotiedot
  stock_level: number;            // Nykyinen saldo
  low_stock_threshold: number;    // Hälytysraja
...
// Lasketut kentät (ei tallennu, lasketaan lennossa)
interface ProductWithMetrics extends Product {
  // Kannattavuus
  gross_margin: number;           // (price - cost) / price
  gross_profit: number;           // price - cost (€)
  
  // Myynti (liitetään orders-datasta)
  units_sold_30d: number;
  revenue_30d: number;
  profit_30d: number;             // (price - cost) * units_sold
  
  // SEO (liitetään GSC-datasta)
...
interface BundleProduct extends Product {
  ...
  bundle_components: {
    ...
    // Komponentin tiedot
    component_cost: number;       // Yksikköhinta
    component_stock: number;      // Komponentin saldo
  }[];
  
  // Lasketut bundle-metriikat
  bundle_metrics: {
    total_component_cost: number; // Kaikkien komponenttien ostohinta yhteensä
    ...
    // Saatavuus (rajoittava tekijä)
    max_assemblable: number;      // Montako pakettia voidaan koota nykyisillä saldoilla
    limiting_component: string;   // Mikä komponentti loppuu ensin
...
interface ProfitMarginIndicator extends BaseIndicator {
  ...
  // Yhteenveto
  summary: {
    total_revenue: number;        // Myynti €
    total_cost: number;           // Ostohinnat yhteensä €
    total_gross_profit: number;   // Kate €
    gross_margin_percent: number; // Kate %
    
    // Trendi
    margin_change_30d: number;    // Muutos edellisestä kuukaudesta
    trend: 'improving' | 'stable' | 'declining';
  };
  ...
  top_profit_products: {
    ...
    profit_per_click: number;     // € katetta per orgaaninen klikkaus
  }[];
...
function calculateProfitMargin(
  ...
  const topProducts = Array.from(productProfits.entries())
    .map(([productId, data]) => {
      ...
      return {
        ...
        organic_clicks: 0,  // Täydennetään GSC-datasta
        profit_per_click: 0
      };
    })
  ...
  const lowMarginProducts = topProducts
    ...
    .map(p => ({
      ...
      recommendation: p.margin_percent < 0 
        ? 'Tuote myydään tappiolla. Nosta hintaa tai poista valikoimasta.'
  ...
  return {
    ...
    alerts: lowMarginProducts.filter(p => p.issue === 'negative_margin').map(p => ({
      ...
      details: `Tuote "${p.product_name}" myydään tappiolla (kate ${p.margin_percent.toFixed(1)}%).`
    }))
  };
}
```
...
interface PackagePerformanceIndicator extends BaseIndicator {
  ...
  value: number;  // Pakettien osuus kokonaismyynnistä (%)
  ...
  packages: {
    ...
    limiting_component: string;   // Mikä rajoittaa
    stock_risk: 'ok' | 'low' | 'critical';
...
interface BundleStockRiskIndicator extends BaseIndicator {
  ...
  value: number;  // € at risk (pakettien myynti joka estyy)
...
| 1.2 | 2025-01-05 | SEO-asiantuntijoiden palaute huomioitu. Lisätty: Organic Conversion Rate, Stock Availability Risk, Brand vs Non-Brand. Uudet alertit. |
| 1.3 | 2025-01-05 | **Billackering.eu -konteksti.** Ostohinnat ja saldot lisätty Product-malliin. Uudet indikaattorit: Profit Margin, Package Performance, Bundle Stock Risk. Todellinen kannattavuusanalyysi mahdollinen. |
...
- Upsell Funnel (Aloituspaketti → Pro → Complete)
