...
-- Migration 004: Add cost_price and RPC functions
-- Date: 2026-01-06
-- =====================================================
...
-- =====================================================
-- 6. CALCULATE SALES METRICS (helper view)
-- =====================================================
CREATE OR REPLACE VIEW v_sales_by_period AS
SELECT
  o.store_id,
  DATE_TRUNC('day', o.creation_date)::date as order_date,
  COUNT(*) as order_count,
  SUM(o.grand_total) as revenue,
  AVG(o.grand_total) as avg_order_value,
  SUM(o.grand_total) / NULLIF(COUNT(*), 0) as calculated_aov
FROM orders o
WHERE o.grand_total > 0
GROUP BY o.store_id, DATE_TRUNC('day', o.creation_date)::date;

-- =====================================================
-- 7. CALCULATE GROSS MARGIN VIEW
-- =====================================================
CREATE OR REPLACE VIEW v_gross_margin_by_product AS
SELECT
  p.store_id,
  p.id as product_id,
  p.name as product_name,
  p.category_name,
  p.price_amount as selling_price,
  p.cost_price,
  CASE
    WHEN p.cost_price IS NOT NULL AND p.price_amount > 0
    THEN ((p.price_amount - p.cost_price) / p.price_amount) * 100
    ELSE NULL
  END as margin_percent,
  CASE
    WHEN p.cost_price IS NOT NULL
    THEN p.price_amount - p.cost_price
    ELSE NULL
